...
from dalib.adaptation.cdan import ConditionalDomainAdversarialLoss, ImageClassifier, MultiLinearMap
...
def main(args: argparse.Namespace):
    ...
    if args.data == 'DomainNet':
        ...
        test_loader = DataLoader(test_dataset, batch_size=args.batch_size*2, shuffle=False, num_workers=args.workers)
    else:
    ...
    # extra loader
    pseudo_selection_dataset = dataset(root=args.root, task=args.target, download=False, transform=val_transform)
    pseudo_selection_loader = DataLoader(pseudo_selection_dataset, batch_size=args.batch_size*2, shuffle=False, num_workers=args.workers)
    unbias_selection_dataset = dataset(root=args.root, task=args.source, download=False, transform=val_transform)
    unbias_selection_loader = DataLoader(unbias_selection_dataset, batch_size=args.batch_size*2, shuffle=False, num_workers=args.workers)
    ...
    for epoch in range(args.epochs):
        ...
        train(train_source_iter, train_target_iter, pseudo_selection_loader, unbias_selection_loader,
                classifier, domain_discri, domain_adv, optimizer, lr_scheduler, epoch, num_classes, args)
    ...
    #save the best model
    if args.save:
        model_path = 'meta/cdan/models/' + time.strftime('%Y-%m-%d')+' '+args.log_filename+'/'
...
def getMetaWeight(x_p: torch.FloatTensor, labels_p: torch.LongTensor,
    ...
    l_g_meta = nn.CrossEntropyLoss(reduction='none')(y_g_hat_source, labels_u)
...
def train(train_source_iter: ForeverDataIterator, train_target_iter: ForeverDataIterator, 
        ...
        model: ImageClassifier, domain_discri: DomainDiscriminator,
          domain_adv: ConditionalDomainAdversarialLoss, optimizer: SGD,
          lr_scheduler: StepwiseLR, epoch: int, class_num: int, args: argparse.Namespace):
    ...
    unbias_bs = args.unbias_batchsize
    if epoch >= args.pre_epochs:
        ...
        unbias_file = select_unbiased_set(
            unbias_selection_loader, model, domain_discri, epoch, class_num, m=args.per_class)
        ...
        unbias_training_dataset = meta_dataset(root="", classes=dataset.CLASSES, data_list_file=unbias_file, transform=val_transform) # prep for testing
        unbias_training_loader = DataLoader(unbias_training_dataset,
                                    batch_size=unbias_bs,shuffle=True,num_workers=args.workers,drop_last=True)

        pseudo_train_iter = ForeverDataIterator(pseudo_training_loader)
        unbias_train_iter = ForeverDataIterator(unbias_training_loader)

        # record all the pseudo weights
        check_path = 'meta/cdan/check/' + time.strftime('%Y-%m-%d')+' '+args.log_filename+'/'
        if not os.path.exists(check_path):
            os.makedirs(check_path)
        check_file = check_path+'epoch-'+str(epoch)+'.txt'
        check = open(check_file, "w")
        check_count = 0
    ...
    for i in range(args.iters_per_epoch):
        ...
        if phase=='meta st':
            ...
            x_p = x_p.to(device)
            labels_p=labels_p.to(device)
            #x_l = torch.cat((x_s, x_p),dim=0)
            #labels_l = torch.cat((labels_s, labels_p),dim=0)
            # w = getMetaWeight(x_p, labels_p, x_u, labels_u, model, class_num)
        ...
        if phase=='meta st':
            ...
            weighted_target_loss = torch.sum(target_loss*w) * args.meta_trade_off
            cls_loss += weighted_target_loss
            
            if check_count < len_train_pseudo:
                for j in range(x_p.size()[0]):
                    check_count += 1
                    check.write(str(path_p[j])+" ")
                    check.write("{:.4f} {:d}\n".format(w[j].item(), labels_p[j].item()))
...
def select_pseudo_label_by_threshold(pseudo_loader, model, iteration):
    ...
    pseudo_path = 'meta/cdan/pseudo/' + time.strftime('%Y-%m-%d')+' '+args.log_filename+'/'
...
def select_unbiased_set(unbias_loader, model, discriminator, iteration, class_num, m=2):
    ...
    unbias_path = 'meta/cdan/unbias/' + time.strftime('%Y-%m-%d')+' '+args.log_filename+'/'
    ...
    with torch.no_grad():
        for i, (inputs, label, path) in enumerate(unbias_loader):
            ...
            ad_input = MultiLinearMap()(features, outputs)
            ad_out = discriminator(ad_input)
            for j, d_score in enumerate(ad_out):
                ad_score = abs(0.5-d_score.cpu().item())
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no_entropy', default=False, action='store_true', help='not using entropy conditioning')
    parser.add_argument('--log_filename', default='log', type=str,
                        help='output file name. ')
    parser.add_argument('--pre_epochs', default=5, type=int,
    ...
    parser.add_argument('--unbias_batchsize', default=8, type=int,
                        help='batchsize for pseudo training. ')
    parser.add_argument('--pseudo_threshold', default=0.95, type=float,
    ...
    train_transform = transforms.Compose([
        ...
        transforms.CenterCrop(224), #visda
